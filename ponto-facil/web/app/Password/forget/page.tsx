  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      const response = await fetch(
        "https://hitpoint-backend-latest.onrender.com/usuarios/forgot-password",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok && responseText) {
        if (
          responseText ===
          `Link de redefinição de senha enviado para o e-mail: ${data.email}`
        ) {
        ...
        setServerError(
          responseText ||
            "Ocorreu um erro ao tentar enviar o link. Tente novamente."
        );
      }
    } catch (error) {
